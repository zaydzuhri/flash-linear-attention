...
@triton.jit
def parallel_softmax_plus_one_attn_fwd_kernel(
    q,
    k,
    v,
    o,
...
@triton.jit
def parallel_softmax_plus_one_attn_bwd_kernel_preprocess(
    o,
    do,
    delta,
    B: tl.constexpr,
...
def parallel_softmax_plus_one_attn_bwd_kernel_dq(
    q,
    k,
    v,
    lse,
...
@triton.jit(do_not_specialize=['T'])
def parallel_softmax_plus_one_attn_bwd_kernel_dkv(
    q,
    k,
    v,
    lse,
...
def parallel_softmax_plus_one_attn_fwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    scale: float,
    ...
    grid = (NV, NT, B * HQ)
    parallel_softmax_plus_one_attn_fwd_kernel[grid](
        q=q,
        k=k,
        v=v,
        o=o,
...
def parallel_softmax_plus_one_attn_bwd_preprocess(
    o: torch.Tensor,
    do: torch.Tensor
):
    V = o.shape[-1]
    delta = torch.empty_like(o[..., 0], dtype=torch.float32)
    parallel_softmax_plus_one_attn_bwd_kernel_preprocess[(delta.numel(),)](
        o=o,
        do=do,
        delta=delta,
...
def parallel_softmax_plus_one_attn_bwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    o: torch.Tensor,
    ...
    delta = parallel_softmax_plus_one_attn_bwd_preprocess(o, do)
    ...
    grid = (NV, NT, B * HQ)
    parallel_softmax_plus_one_attn_bwd_kernel_dq[grid](
        q=q,
        k=k,
        v=v,
        lse=lse,
    ...
    parallel_softmax_plus_one_attn_bwd_kernel_dkv[grid](
        q=q,
        k=k,
        v=v,
        lse=lse,
...
@torch.compile
class ParallelSoftmaxPlusOneAttentionFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, scale, offsets):
        ...
        o, lse = parallel_softmax_plus_one_attn_fwd(
            q=q,
            k=k,
            v=v,
            scale=scale,
    ...
    def backward(ctx, do):
        ...
        dq, dk, dv = parallel_softmax_plus_one_attn_bwd(
            q=q,
            k=k,
            v=v,
            o=o,
...
def parallel_softmax_plus_one_attn(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    o = ParallelSoftmaxPlusOneAttentionFunction.apply(q, k, v, scale, cu_seqlens)
    if head_first:
