...
@triton.jit
def parallel_abs_softmax_attn_fwd_kernel(
    q,
    k,
    v,
    o,
    ...
    for i_s in range(i_t * BT, min((i_t + 1) * BT, T), BS):
        ...
        if MODE == 2:
            b_p = exp(b_s - b_m[:, None])
            b_p_r = b_p
            b_p_a = tl.where(o_q[:, None] >= o_k[None, :], b_p, 0)
        else:
            b_p = exp(b_s - b_m[:, None]) - exp(-b_m[:, None])
            b_p_a = tl.abs(b_p)
...
@triton.jit
def parallel_abs_softmax_attn_bwd_kernel_preprocess(
    o,
    do,
    delta,
    B: tl.constexpr,
...
def parallel_abs_softmax_attn_bwd_kernel_dq(
    q,
    k,
    v,
    lse,
    ...
    for i_s in range(0, i_t * BT, BS):
        ...
        # [BT, BS]
        if MODE == 2:
            b_ds = b_e * (b_dp - b_delta[:, None])
        else:
            b_step = tl.where(b_s > 0, b_dp, -b_dp)
...
@triton.jit(do_not_specialize=['T'])
def parallel_abs_softmax_attn_bwd_kernel_dkv(
    q,
    k,
    v,
    lse,
    ...
    for i_s in range(i_t * BT, min((i_t + 1) * BT, T), BS):
        ...
        b_e = exp(b_s - b_lse[None, :])
        if MODE == 2:
            b_p_r = tl.where(o_k[:, None] <= o_q[None, :], b_e, 0)
        else:
        ...
        if MODE == 2:
            ...
            b_step = tl.where(b_p > 0, b_dp, -b_dp)
            b_sign = tl.where(b_p > 0, b_delta[None, :], -b_delta[None, :])
...
def parallel_abs_softmax_attn_fwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    scale: float,
    ...
    grid = (NV, NT, B * HQ)
    parallel_abs_softmax_attn_fwd_kernel[grid](
        q=q,
        k=k,
        v=v,
        o=o,
...
def parallel_abs_softmax_attn_bwd_preprocess(
    o: torch.Tensor,
    do: torch.Tensor
):
    V = o.shape[-1]
    delta = torch.empty_like(o[..., 0], dtype=torch.float32)
    parallel_abs_softmax_attn_bwd_kernel_preprocess[(delta.numel(),)](
        o=o,
        do=do,
        delta=delta,
...
def parallel_abs_softmax_attn_bwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    o: torch.Tensor,
    ...
    delta = parallel_abs_softmax_attn_bwd_preprocess(o, do)
    ...
    grid = (NV, NT, B * HQ)
    parallel_abs_softmax_attn_bwd_kernel_dq[grid](
        q=q,
        k=k,
        v=v,
        lse=lse,
    ...
    parallel_abs_softmax_attn_bwd_kernel_dkv[grid](
        q=q,
        k=k,
        v=v,
        lse=lse,
...
@torch.compile
class ParallelAbsSoftmax1AttentionFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, scale, offsets):
        ...
        o, lse = parallel_abs_softmax_attn_fwd(
            q=q,
            k=k,
            v=v,
            scale=scale,
    ...
    def backward(ctx, do):
        ...
        dq, dk, dv = parallel_abs_softmax_attn_bwd(
            q=q,
            k=k,
            v=v,
            o=o,
...
@torch.compile
class ParallelAbsSoftmax2AttentionFunction(torch.autograd.Function):
...
def parallel_abs_softmax_1_attn(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    o = ParallelAbsSoftmax1AttentionFunction.apply(q, k, v, scale, cu_seqlens)
    if head_first:
        o = rearrange(o, 'b t h d -> b h t d')
    return o


def parallel_abs_softmax_2_attn(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    scale: Optional[float] = None,
    ...
    if head_first:
        q, k, v = map(lambda x: rearrange(x, 'b h t d -> b t h d'), (q, k, v))
    o = ParallelAbsSoftmax2AttentionFunction.apply(q, k, v, scale, cu_seqlens)
    if head_first:
        o = rearrange(o, 'b t h d -> b h t d')
    return o
