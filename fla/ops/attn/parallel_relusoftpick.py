...
def parallel_softpick_attn_fwd_kernel(
    ...
    for i_s in range(i_t * BT, min((i_t + 1) * BT, T), BS):
        ...
        b_p = exp(b_s - b_m[:, None]) - exp(-b_m[:, None])
        b_p_r = tl.maximum(b_p, 0.0)
        b_p_a = tl.where(o_q[:, None] >= o_k[None, :], b_p_r, 0)
        # [BT]
        b_acc = b_acc * b_r + tl.sum(b_p_a, 1)
        # [BT, BV]
        b_o = b_o * b_r[:, None] + tl.dot(b_p_r.to(b_q.dtype), b_v)
...
def parallel_softpick_attn_bwd_kernel_dq(
    ...
    for i_s in range(0, i_t * BT, BS):
        ...
        b_step = tl.where(b_p > 0, b_dp, 0)
        b_sign = tl.where(b_p > 0, b_delta[:, None], 0)
...
@torch.compile
class ParallelReluSoftpick1AttentionFunction(torch.autograd.Function):
...
def parallel_relu_softpick_1_attn(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    o = ParallelReluSoftpick1AttentionFunction.apply(q, k, v, scale, cu_seqlens)
    if head_first:
