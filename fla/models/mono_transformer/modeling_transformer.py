...
class MonoTransformerPreTrainedModel(PreTrainedModel):

    config_class = MonoTransformerConfig
...
class MonoTransformerModel(MonoTransformerPreTrainedModel):

    def __init__(
        self,
        config: MonoTransformerConfig
        ...
        self.layers = nn.ModuleList([MonoTransformerBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.norm = (RMSNorm if config.fuse_norm else nn.RMSNorm)(config.hidden_size, eps=config.norm_eps)
...
class MonoTransformerForCausalLM(MonoTransformerPreTrainedModel, FLAGenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = MonoTransformerModel(config)
        self.vocab_size = config.vocab_size
