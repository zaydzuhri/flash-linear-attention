...
from fla.ops.attn import parallel_softpick_attn
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        plot_name="softpick_performance",
        args={},
    )
)
def benchmark(T, provider):
    ...
    if provider == "triton_fwd":
        return triton.testing.do_bench(
            lambda: parallel_softpick_attn(q, k, v, head_first=False),
            quantiles=quantiles,
        )
    if provider == "ref_fwd":
        return triton.testing.do_bench(
            lambda: naive_softpick_attn(q, k, v, head_first=False)[0],
            quantiles=quantiles,
        )
    if provider == "triton_fwdbwd":
        return triton.testing.do_bench(
            lambda: parallel_softpick_attn(q, k, v, head_first=False).backward(do),
            quantiles=quantiles,
        )
    if provider == "ref_fwdbwd":
        return triton.testing.do_bench(
            lambda: naive_softpick_attn(q, k, v, head_first=False)[0].backward(do),
            quantiles=quantiles,
        )
...
def print_memory_table():
    ...
    providers = [
        ("triton_fwd", lambda q, k, v, do: parallel_softpick_attn(q, k, v, head_first=False)),
        ("ref_fwd", lambda q, k, v, do: naive_softpick_attn(q, k, v, head_first=False)[0]),
        ("triton_fwdbwd", lambda q, k, v, do: parallel_softpick_attn(q, k, v, head_first=False).backward(do)),
        ("ref_fwdbwd", lambda q, k, v, do: naive_softpick_attn(q, k, v, head_first=False)[0].backward(do)),
    ]
    x_vals = [64] + [128 * 2 ** i for i in range(0, 6)]
    dtype = torch.bfloat16
    B, H, D = 4, 8, 64
